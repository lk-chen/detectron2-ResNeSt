...
def fast_rcnn_inference_single_image(
    ...
    print('### fast_rcnn_inference_single_image')
    """
...
class FastRCNNOutputs(object):
    ...
    def __init__(
        ...
        self.handle_proposals(proposals)
        
        if only_train_cyclist:
            # Ignore proposals that are not cyclist nor background
            if self.gt_classes is not None:
                indices = self.get_indices(self.gt_classes)
                box_indices = self.get_bool_indices(self.gt_classes)
                ...
                for proposal in proposals:
                    if proposal.has("gt_boxes"):
                        local_indices = self.get_indices(proposal.gt_classes)
                        local_box_indices = self.get_bool_indices(proposal.gt_classes)
                        proposal.gt_boxes = proposal.gt_boxes[local_box_indices]
                        proposal.gt_classes = proposal.gt_classes[local_indices]
                        proposal.proposal_boxes = proposal.proposal_boxes[local_box_indices]
                        sliced_proposals.append(proposal)
                self.proposals = sliced_proposals
 
                self.box2box_transform = box2box_transform
                self.num_preds_per_image = [len(p) for p in sliced_proposals]
                self.pred_class_logits = pred_class_logits[indices]
                self.pred_proposal_deltas = pred_proposal_deltas[indices]
                self.smooth_l1_beta = smooth_l1_beta
                self.image_shapes = [x.image_size for x in sliced_proposals]
                self.handle_proposals(sliced_proposals)
 
        self._no_instances = len(proposals) == 0  # no instances found
 
    def get_bool_indices(self, gt_classes):
        CYCLIST_INDICES = 5
        BG_INDEX = 10
        return ((gt_classes == CYCLIST_INDICES) | (gt_classes == BG_INDEX)).view(-1)
 
    def get_indices(self, gt_classes):
        return self.get_bool_indices(gt_classes).nonzero().view(-1)
    
    def handle_proposals(self, proposals):
        if len(proposals):
